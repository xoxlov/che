...
def test_birthday_default(
    ...
    assert page.is_driver_1_birthday_displayed()
    assert page.get_driver_1_birthday() == ""
    assert not page.is_driver_1_birthday_checked_good()


def test_driver_1_birthday(
    ...
    page.set_driver_1_birthday_date(data_osago_date_birth)
    counter_after = page.get_values_to_enter_count()
    if data_osago_date_birth.valid:
        assert page.is_driver_1_birthday_checked_good()
        assert counter_before - 1 == counter_after
    else:
        assert not page.is_driver_1_birthday_checked_good()
        assert counter_before == counter_after
        assert page.date_driver_1_birthday_error_message
...
def test_one_character_and_jump(
    ...
    """
    page = osago_web_app.calculator_page
    page.date_driver_1_birthday_send_keys("1" + Keys.BACK_SPACE + Keys.TAB)
    assert not page.date_driver_1_birthday_error_message
    assert not page.is_driver_1_birthday_checked_good()
    assert page.get_driver_1_birthday() == ""
...
def test_one_character_and_stand(
    ...
    """
    page = osago_web_app.calculator_page
    page.date_driver_1_birthday_send_keys("1")
    assert not page.date_driver_1_birthday_error_message
    assert not page.is_driver_1_birthday_checked_good()
...
def test_multi_characters_and_stand(
    ...
    page.date_driver_1_birthday_send_keys(date[:-1])
    assert not page.date_driver_1_birthday_error_message
    assert not page.is_driver_1_birthday_checked_good()
...
def test_all_characters_and_stand(
    ...
    page = osago_web_app.calculator_page
    date = (datetime.today() - relativedelta(years=30)).strftime("%d.%m.%Y")
    page.date_driver_1_birthday_send_keys(date)
    assert not page.date_driver_1_birthday_error_message
    assert not page.is_driver_1_birthday_checked_good()
